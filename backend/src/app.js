...
export function createApp({database=db,sessionStore,transport}={}){
 ...
 app.get(`/api/admin/${key}`,async(_req,res)=>{
   const [rows]=await database.query(`SELECT r.*,COALESCE((SELECT jsonb_agg(jsonb_build_object('kind',n.kind,'state',n.state,'attempts',n.attempts,'last_error',n.last_error,'sent_at',n.sent_at,'next_attempt_at',n.next_attempt_at) ORDER BY n.id) FROM notification_jobs n WHERE n.request_type=$1 AND n.request_id=r.id),'[]'::jsonb) AS notifications FROM ${table} r ORDER BY r.created_at DESC LIMIT 1000`,[key]);res.json(rows);
 });
 ...
 app.use((err,_req,res,_next)=>{const errors={23505:[409,'Cet identifiant existe déjà.'],23503:[422,'Un contenu associé est introuvable ou encore utilisé.'],LIMIT_FILE_SIZE:[413,'L’image dépasse 5 Mo.']};const mapped=errors[err.code];const status=err.status||mapped?.[0]||500;if(status===500)console.error('API :',err.code||err.name||'ERROR');res.status(status).json({message:mapped?.[1]||(status===500?'Service momentanément indisponible. Réessayez plus tard.':err.message),fields:err.fields});});
 return app;
}
